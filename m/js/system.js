	...
	'flickity':function(){//手机滑动
		if($(".ff-gallery1,ff-gallery2,ff-gallery3,ff-gallery4,ff-gallery5,ff-gallery6,ff-gallery7,ff-gallery8,ff-gallery9,ff-gallery10,ff-gallery11,ff-gallery12,ff-gallery13,ff-gallery14,ff-gallery15").length){
			...
			$("<link>").attr({
				rel: "stylesheet",
				type: "text/css",
				href: "//www.77kpw.net/template/77shouji/css/flickity.min.css"
			}).appendTo("head");
			$.getScript("//www.77kpw.net/template/77shouji/js/flickity.pkgd.min.js", function(){
...
'image': {//图片
	'lazyload': function(){//延迟加载
		...
		$.getScript("//www.77kpw.net/template/77shouji/js/jquery.lazyload.js", function(response, status) {
			
			$("img.ff-img").lazyload({
				placeholder : "//www.77kpw.net/template/77shouji/images/no.jpg",
				effect : "fadeIn",
				failurelimit: 15
				//threshold : 400
	...
	'qrcode': function(){//生成二维码
		...
		$(".glyphicon-phone").on('show.bs.popover', function () {
			$(".glyphicon-phone").attr('data-content','<img class="ff-qrcode" src="//www.77kpw.net/qrcode/api.php?v='+encodeURIComponent(feifei.browser.url)+'"/>');
		})
	},
	'vcode':function(){//安全码
...
'search': {//搜索
	...
	'autocomplete': function(){//3.5增加搜索结果控制
		...
		if( $limit > 0){
			...
			$.getScript("//www.77kpw.net/template/77shouji/js/jquery.autocomplete.min.js", function(response, status) {
